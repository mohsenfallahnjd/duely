					...
					<div key={`${year}-${month}`}>
						...
						<div className="flex items-center justify-between mb-2 px-1">
							<div className="flex items-center gap-2">
								<h3
									className={`font-semibold text-zinc-900 dark:text-white ${fa ? "font-[family-name:var(--font-vazirmatn)]" : ""}`}
								>
									{monthLabel}
								</h3>
								{isCurrent && (
						...
						<div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 divide-y divide-zinc-100 dark:divide-zinc-800 overflow-hidden">
							{monthLoans.map((loan) => {
								...
								const isOverdue =
									isCurrent && !paid && now.getDate() > loan.dueDay;
								...
								return (
									...
									>
										<button
											type="button"
											onClick={() =>
												void handleToggle(loan.id, year, month, !paid)
											}
											disabled={isLoading}
											className={cn(
