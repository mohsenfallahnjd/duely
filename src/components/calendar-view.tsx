...
function CalendarInner({
...
}: { loans: Loan[]; allPayments: Payment[] }) {
	...
	useEffect(() => {
		if (cal === "jalali") {
			...
			setDispYear(j.year);
			setDispMonth(j.month);
		} else {
			setDispYear(now.getFullYear());
			setDispMonth(now.getMonth() + 1);
		}
	...
	const { year: dispGYear, month: dispGMonth } =
		cal === "jalali"
			? fromJalali(dispYear, dispMonth, 15)
			: { year: dispYear, month: dispMonth };

	function loanActiveForGMonth(
		loan: Loan,
		gYear: number,
		gMonth: number,
	): boolean {
	...
	function loansDueOnDay(day: number): Array<{ loan: Loan; paid: boolean }> {
		return loans
			.filter(
				(l) =>
					l.dueDay === day && loanActiveForGMonth(l, dispGYear, dispGMonth),
			)
			.map((l) => {
				const payment = allPayments.find(
					(p) =>
						p.loanId === l.id && p.year === dispGYear && p.month === dispGMonth,
				);
	...
	const visibleLoans = loans.filter((l) =>
		loanActiveForGMonth(l, dispGYear, dispGMonth),
	);
		...
		>
			...
			<header className="border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur sticky top-0 z-10">
				<div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
					<Link
						href="/"
						className="p-2 -ml-2 text-zinc-500 hover:text-zinc-900 dark:hover:text-white rounded-lg transition"
					>
			...
			<main className="max-w-2xl mx-auto w-full px-4 py-6 flex flex-col gap-6">
				...
				<div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
					...
					<div className="grid grid-cols-7 border-b border-zinc-100 dark:border-zinc-800">
						{weekDays.map((d) => (
							<div
								key={d}
								className="py-2.5 text-center text-xs font-medium text-zinc-400 dark:text-zinc-500"
							>
								{d}
							</div>
						))}
				...
				{visibleLoans.length === 0 && (
					<div className="text-center py-12 text-zinc-400 dark:text-zinc-500 text-sm">
						{fa
							? "هیچ سررسیدی در این ماه وجود ندارد"
