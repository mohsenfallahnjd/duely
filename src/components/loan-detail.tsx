...
import { ArrowLeft, Check, ExternalLink, Pencil, TrendingDown } from "lucide-react";
import Link from "next/link";
import { cn } from "@/lib/cn";
import { CalendarProvider, useCalendar } from "./calendar-context";
import { EditLoanModal } from "./edit-loan-modal";
import { toJalali, GREGORIAN_MONTHS, JALALI_MONTHS, getInstallmentLabel, formatDueDay, isCurrentMonth } from "@/lib/calendar";
...
function LoanDetailInner({ loan, initialPayments }: { loan: Loan; initialPayments: Payment[] }) {
  ...
  const curTotal = now.getFullYear() * 12 + now.getMonth();
  const paidCount = months.filter(({ year, month }) => paymentMap.get(`${year}-${month}`)?.paid).length;
  ...
  const allDone = paidCount === months.length && months.length > 0;
    ...
    <div className={`min-h-screen flex flex-col bg-zinc-50 dark:bg-zinc-950 ${fa ? "font-[family-name:var(--font-vazirmatn)]" : ""}`} dir={fa ? "rtl" : "ltr"}>
      <header className="border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href="/dashboard" className="p-2 -ms-2 text-zinc-500 hover:text-zinc-900 dark:hover:text-white rounded-lg transition">
      ...
      <main className="max-w-2xl mx-auto w-full px-4 py-5 pb-12 flex flex-col gap-4">
        {/* Summary card */}
        <div className={cn(
          "rounded-3xl p-5",
          allDone
            ? "bg-emerald-900 dark:bg-emerald-950 text-emerald-50"
            : "bg-zinc-900 dark:bg-zinc-800 text-white",
        )}>
          {/* Header row */}
          <div className="flex items-start justify-between gap-3 mb-4">
            <div>
              <p className="text-xs font-medium opacity-50 uppercase tracking-wide mb-1">
                {fa ? "هر قسط" : "Per installment"}
              </p>
              <p className="text-2xl font-bold tabular-nums">
                {loanState.amount.toLocaleString()}
                <span className="text-base font-normal opacity-50 ms-1.5">{loanState.currency}</span>
              </p>
            </div>
            <div className="text-end">
              <p className="text-xs font-medium opacity-50 uppercase tracking-wide mb-1">
                {fa ? "اقساط" : "Installments"}
              </p>
              <p className="text-2xl font-bold tabular-nums">
                {paidCount}<span className="opacity-40 font-normal">/{months.length}</span>
              </p>
            </div>
          </div>
          ...
          <div className="h-1.5 rounded-full bg-white/10 overflow-hidden mb-3">
            <div
              className={cn(
                ...
                allDone ? "bg-emerald-400" : "bg-white/70",
              )}
          ...
          <div className="flex items-center justify-between text-xs opacity-60">
            <span>{fa ? "پرداخت شده" : "Paid"}: {paidAmt.toLocaleString()} {loanState.currency}</span>
            <span>
              {allDone
                ? fa ? "✓ تمام شد" : "✓ Complete"
                : `${fa ? "باقی" : "Left"}: ${remaining.toLocaleString()} ${loanState.currency}`}
            </span>
          </div>

          {/* Due day + pay link */}
          <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
            <span className="text-xs opacity-50">{formatDueDay(loanState.dueDay, cal, lang)}</span>
            {loanState.paymentUrl && (
              <a
                href={loanState.paymentUrl}
                target="_blank"
        ...
        <div className="rounded-2xl bg-white dark:bg-zinc-900 ring-1 ring-zinc-100 dark:ring-zinc-800 overflow-hidden">
          <div className="px-4 py-3 border-b border-zinc-50 dark:border-zinc-800">
            <h2 className="text-xs font-medium uppercase tracking-widest text-zinc-400 dark:text-zinc-500">
              {fa ? "تاریخچه اقساط" : "Payment history"}
            </h2>
          </div>
          <div className="divide-y divide-zinc-50 dark:divide-zinc-800">
            {months.map(({ year, month }, idx) => {
              const payment = paymentMap.get(`${year}-${month}`);
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isCurrent && !paid && "bg-zinc-50 dark:bg-zinc-800/40",
                    paid && "opacity-45",
                  )}
                >
                  <button
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {loanState.installments && (
                      <span className="ms-1.5 text-xs text-zinc-300 dark:text-zinc-700 tabular-nums">
                        {idx + 1}/{loanState.installments}
                      </span>
                    )}
                    {isOverdue && (
                      <span className="ms-1.5 text-xs text-red-400">{fa ? "· تأخیر" : "· overdue"}</span>
                    )}
                  </div>
                  ...
                  <div className="shrink-0 text-end">
                    {paid && payment?.paidAt ? (
                      ...
                      <span className={cn(
                        ...
                        isOverdue ? "text-red-400" : isFuture ? "text-zinc-300 dark:text-zinc-700" : "text-zinc-400 dark:text-zinc-500",
                      )}>
                        {loanState.amount.toLocaleString()}
                      </span>
                    )}
