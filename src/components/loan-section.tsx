			...
			<div className="flex items-start justify-between px-4 py-3.5 gap-3">
				...
				>
					...
					<div className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">
						...
						{" · "}
						{formatDueDay(loan.dueDay, cal, lang)}
						{loan.installments && (
							<span>
								{" "}
								·{" "}
					...
					{months.map(({ year, month }, idx) => {
						...
						return (
							...
							>
								...
								>
									{isLoading ? (
										<span className="w-2 h-2 rounded-full border-2 border-zinc-300 border-t-zinc-900 dark:border-t-white animate-spin" />
									) : paid ? (
										<Check
								...
								<div className="flex-1 min-w-0">
									...
									{loan.installments && (
										<span className="ms-1.5 text-xs text-zinc-300 dark:text-zinc-600 tabular-nums">
											{installNum}/{loan.installments}
										</span>
									)}
									{isOverdue && (
										<span className="ms-1.5 text-xs text-red-400">
											{fa ? "· تاخیر" : "· overdue"}
										</span>
									)}
